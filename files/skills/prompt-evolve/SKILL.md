---
name: prompt-evolve
description: Author a self-improving, version-controlled prompt for tough tasks done in many iterative passes over partitions. Use when the work doesn't fit in one execution, the partitions are obvious (year, season, module, batch, subsystem), each pass produces both work AND insight, and the insight would otherwise be lost between passes. Triggers on phrases like "evolving prompt", "mining prompt", "self-improving prompt", "iterative population", "we'll process this in batches and learn as we go", "back-fill this corpus over time". Primary mode is `--from <SPEC>` — promotes an evolving-prompt deliverable embedded in a `/spec` into a standalone artifact at `<docs|project/agents>/prompt-evolve/<NAME>_PROMPT.md`. Enforces a 7-phase scaffold (Pre-flight → Inventory → Extract → Write → Verify → Report → Self-Improvement) with the Phase 6 self-edit mandate, CHANGELOG entry format, tool-failure fallback matrix, pending-re-run staging in the report, idempotence guarantee, and absolute-path-inline rule. Do NOT use for single-execution work (use `/spec`), single-paragraph capture (`/note`), parallel-workstream decomposition (`/program`), or executor-round verification (`/round-review`). The artifact this produces evolves over its lifetime — Phase 6 self-edits accumulate lessons each pass, and the prompt's CHANGELOG IS its institutional memory.
---
