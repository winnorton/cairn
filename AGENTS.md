...
- `packages/cairn-claude/` — the `cairn` Claude Code plugin shipping all skills to
  Claude Code (and to agy via `agy plugin import claude`). Skills synced from
  `files/skills/` by the shared `scripts/sync-skills.mjs`; drift guard runs in CI
  (plugin has no `prepublishOnly`). Per `[LAW own-your-namespace]`.
