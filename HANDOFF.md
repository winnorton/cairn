...
- **Latest release:** v0.14.0 (`agents/` umbrella + git as memory bus. Memory + laws move out of `.claude/` and `~/.claude/memory/` into `<project>/agents/` — cairn-controlled namespace, vendor-neutral, name encodes scope. Drops slug policy and cross-slug HANDOFF pointers; repo boundary IS memory boundary. `git push` / `git pull` is the cross-machine sync; `git log -- agents/memory/` is the activity feed. Skills stay at `~/.claude/skills/` because the loader requires that path; cairn-format state is all under `agents/` now. Migration: re-adopters move `.claude/LAWS.md` → `agents/LAWS.md`, `.claude/cairn-version` → `agents/cairn-version`, optionally copy user-space memory into `agents/memory/`. See `adopt.md` migration section. Posture per `[MEM project/cairn-blend-strategy-pillars]`: stay above storage in a vendor-neutral home.)
- **Previous release:** v0.13.1 (Renames `/review` to `/peer-review` to disambiguate from Claude Code's built-in `/review` skill — bare `/review` was being silently shadowed.)
- **Earlier:** v0.13.0 (Slug-only law identity — drops Law N numbering). v0.12.3 (Doc patch — fixed v0.12.x consistency gaps that fresh `/peer-review` caught). v0.12.2 (install-report version strings). v0.12.1 (skill format migration to canonical subdir form + `/peer-review` skill + folder-as-state for `plans/`). v0.12.0 (`/note` + `/spec` artifact-creation skills). v0.11.3 (ephemeral-sandbox pre-flight in adopt.md).
