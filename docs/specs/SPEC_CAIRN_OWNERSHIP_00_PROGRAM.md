...
├── sessions/            # (reserved — HIVE-owned) per-project session federation (see §2.6)
└── cairn-version        # single-line version marker
```
All cairn state is git-tracked inside the adopter's repo. No `~/.claude`, `~/.gemini`, `~/.pi`, or `.agents/` path is ever a cairn write target. **`<project>/CLAUDE.md` is fully user-owned** — cairn never writes or seeds it; the user creates it (if absent) and adds the one import line (§2.2). The manifest carries NO `{projectRoot}/CLAUDE.md` entry. `.cairn/sessions/` is reserved for the HIVE effort; WS01 creates the `.cairn/` skeleton but does not own or populate `sessions/`.
...
**2.4 Single source of truth (frozen by WS11).** `files/skills/<name>/SKILL.md` is canonical. Every package's skill copy is a byte-identical committed artifact regenerated by a shared `scripts/sync-skills.mjs`; `--check` fails on drift, over-cap descriptions, and version-lockstep breaks. **Each package declares its version-source file in `sync-config.json` via a `versionFile` key** — `package.json` for npm packages (`cairn-pi`), `plugin.json` for the non-npm `cairn` Claude plugin (which ships NO `package.json`). The guard wires to `prepublishOnly` for npm packages and to a **repo CI check** for non-npm packages (a plugin has no `prepublishOnly`). (Generalizes `packages/cairn-pi/scripts/sync-skills.mjs`.)

**2.5 Path-variable semantics (frozen by WS06).** `manifest.json` describes `.cairn/` state dests via `{projectRoot}/.cairn/...` AND updates each state file's `src` to `files/.cairn/...` in lockstep (WS01 moved the template tree there; WS08's fetch reads `src`, so a stale `src` 404s). The `userMemory` vendor-path variable and the `{projectRoot}/CLAUDE.md` entry are both retired. `{projectRoot}` resolves per-harness to the adopter's repo root.

**2.6 HIVE_CONTEXT_SESSIONS coordination (cross-effort, doc-level only).** Per [`NOTE_HIVE_OWNERSHIP_COORDINATION_2026-06-13`](../notes/NOTE_HIVE_OWNERSHIP_COORDINATION_2026-06-13.md): HIVE ships in separate repos (`cairn-mcp-server`, `cairn-sessions`) this program does not touch, but overlaps at four cairn-repo surfaces. (a) `.cairn/sessions/` is HIVE's realigned per-project path (off the dead `agents/` umbrella) — reserved in §2.1, not OWNERSHIP-owned. (b) **WS07** must PRESERVE the "Session corpus and findings ledger" section in `files/skills/session-distill/SKILL.md` (corpus integration, not a vendor-path probe). (c) **WS10** must CARRY FORWARD the HIVE notes in `AGENTS.md`/`HANDOFF.md` (corpus, `cairn-mcp-server` v0.5.0, ingest CLI). (d) **WS12** inventories the HIVE cairn-namespaced artifacts (the two repos, the `cairn-ingest` bin, the MCP tool names) and confirms against `[LAW own-your-namespace]` — likely zero renames. OWNERSHIP's §1 non-goal (cross-project shared memory) keeps `.cairn/memory/` and the HIVE corpus coherent and separate.
...
| **cairn-claude packaging** (spec-review B1) | Non-npm plugin: `plugin.json` only, **NO `package.json`**; version via `sync-config.json` `versionFile: plugin.json`; guard runs in **CI**, not `prepublishOnly` | Claude Code plugins aren't npm packages; a `package.json` solely to satisfy the guard is cargo-cult. The shared guard reads the declared `versionFile`. Resolves the WS03↔WS11 manifest-filename conflict. |
| **`<project>/CLAUDE.md` seeding** (spec-review) | NOT seeded — fully user-owned; manifest carries no `{projectRoot}/CLAUDE.md` entry | The one vendor-file touch is the user-written import line (§2.2); cairn seeding `<project>/CLAUDE.md` would contradict §4. WS06 drops the entry; WS08 never writes it. |
| **Completed child-spec archive dir** (spec-review) | `docs/specs/archive/` for completed children; `_promoted/` reserved for the superseded umbrella ONLY | Don't conflate "completed" with "superseded." WS07/WS08 use `archive/` like the other children. |
| **`.cairn/sessions/` ownership** (HIVE) | HIVE-owned, OWNERSHIP-reserved; WS01 creates the `.cairn/` skeleton, not `sessions/` | Cross-effort coordination (§2.6); prevents clobbering the shared `.cairn/` namespace. |
...
1. `rg -n "~/.claude|~/.gemini|~/.pi/agent|projectClaude}/LAWS|userMemory" adopt.md manifest.json files/.cairn/ | rg -v "migration-ref"` returns **zero** — no cairn STATE file, the install flow, or the manifest names a vendor write target outside a `<!-- migration-ref -->` line. (Scope is `files/.cairn/`, the cairn-owned state tree this write-guard protects — NOT `files/skills/`: skill *bodies* legitimately READ vendor paths (transcript locations, v0.13.x memory-fallback probes, distillate-transport notes) and are exempt per §4.)
...
### §9.5 Integration order (from spec-review merge-map — Pass 3)

Shared real files have a single writer or a strict serialize order. The code phase integrates in this order so parallel worktrees never clobber:

| Shared file | Writer(s) → order | Note |
|---|---|---|
| `files/.cairn/CLAUDE.md` | WS01 (owns body via `git mv`) → WS02 (prepend import-note header, never overwrite) | NOT parallel-safe despite both Wave 0 |
| `manifest.json` | WS06 ONLY (after WS01 moves `files/.cairn/`) | WS06 updates `src` AND `dest`; WS08 reads `src` |
| `adopt.md` | WS04 (agy Step text) → WS08 (wholesale rewrite) → WS09 (append migration §) | WS09 sole `<!-- migration-ref -->` owner |
| `packages/cairn-claude/**` | WS03 (plugin.json + standalone script) → WS11 (replace script w/ shared-guard delegate) | `versionFile: plugin.json`, CI-check not prepublish (§2.4) |
| `packages/cairn-pi/**` | WS05 (README + version bump) → WS11 (scripts) | serialize WS05 → WS11 |
| `files/skills/*/SKILL.md` | WS07 (memory-path sweep) + any WS12 rename → THEN WS03/05/11 regenerate package copies | else committed copies drift, guard goes red |
| `VERSION` (root) | one owner bumps to `0.14.0`; WS01 `cairn-version`, WS03/05/11 lockstep, WS06 manifest version must all match; `HANDOFF.md` same commit (DoD#9) | assign the bump to the final integration commit |
| `README.md` · `AGENTS.md` · `HANDOFF.md` · root `CLAUDE.md` | WS10 ONLY (last; reflects WS12 renames) | |
